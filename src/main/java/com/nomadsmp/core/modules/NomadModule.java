...
public class NomadModule {
    ...
    public void enable() {
        long intervalTicks = plugin.getConfigManager().getMigrationCheckMinutes() * 60L * 20L;
        taskId = Bukkit.getScheduler().runTaskTimer(plugin, this::checkMigration, intervalTicks, intervalTicks).getTaskId();
    }
    ...
    private void checkMigration() {
        ...
        if (now.getDayOfWeek() == config.getMigrateDay() && now.getHour() == config.getMigrateHour()) {
            long warnTicks = config.getMigrationWarningMinutes() * 60L * 20L;
            Bukkit.broadcastMessage("\u00a7c\u00a7l\u26a0 THE WORLD SHIFTS IN " + config.getMigrationWarningMinutes() + " MINUTES. \u26a0 \u00a7r\u00a7ePrepare your home!");
            Bukkit.getScheduler().runTaskLater(plugin, this::runMigration, warnTicks);
        }
    }
    ...
    public void runMigration() {
        ...
        int radius = config.getWorldBorderSize() / 2;
        ...
        for (Map.Entry<UUID, Location> entry : homes.entrySet()) {
            ...
            for (int attempt = 0; attempt < 10; attempt++) {
                ...
                newHome = SafeLocationFinder.findSafe(world, nx, nz, config.getMinSafeY());
    ...
    private void copyPasteSchematic(Location from, Location to, int radius) throws WorldEditException {
        ...
        Operation operation = holder.createPaste(editSession)
            ...
            .copyEntities(false).copyBiomes(true).build();
